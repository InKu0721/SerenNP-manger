...
func (a *App) ImportPOC(content string, category string) (*models.POCTemplate, error) {
	...
	if template.Name != "" {
		...
		if uniqueName != template.Name {
			// 更新模板名称（保留原始 YAML 内容，只替换 info.name 字段）
			template.Name = uniqueName
			// 在原始 content 中替换 name 字段（保留完整的 HTTP 请求配置等）
			if template.Content != "" {
				template.Content = replaceYAMLName(template.Content, uniqueName)
			}
		}
	}
...
func (a *App) GetStats() (*models.Stats, error) {
	...
	stats := &models.Stats{
		TotalPOCs:      len(pocs),
		SecurityScore:  100, // 默认值，可后续根据实际漏洞计算
		ByCategory:     make(map[string]int),
		BySeverity:     make(map[string]int),
		SeverityCounts: make(map[string]int),
		RecentScans:    []models.ScanStatus{},
	}
	...
	// 取最近 5 个扫描任务
	if len(scans) > 5 {
		stats.RecentScans = scans[len(scans)-5:]
	} else {
...
func (a *App) SaveSettings(settings models.Settings) error {
	...
	settingsDir := filepath.Join(homeDir, ".nuclei-poc-manager")
...
func (a *App) LoadSettings() (*models.Settings, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			templatesDir := filepath.Join(homeDir, ".nuclei-poc-manager", "templates")
			if a.pocManager != nil {
	...
	// 如果加载的设置中没有模板目录，使用默认值
	if settings.TemplatesDir == "" && a.pocManager != nil {
		settings.TemplatesDir = a.pocManager.GetTemplatesDir()
	}
