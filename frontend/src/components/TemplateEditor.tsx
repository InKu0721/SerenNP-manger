...
function TemplateEditor({ template, onClose, onSave }: TemplateEditorProps) {
  ...
  }, [template?.id]) // 当切换编辑不同模板时重新解析

  // 从代码模式切换回可视化时，重新解析代码编辑器中修改的内容
  useEffect(() => {
    if (mode === 'visual' && content) {
...
`
  ...
  useEffect(() => {
    if (mode === 'code') {
      ...
      setContent(newYAML)
      validateYAML(newYAML) // 直接验证新构建的 YAML，而不是等 state 更新
    }
  }, [mode])
    ...
    try {
      ...
      if (template?.id) {
        ...
        if (window.go?.main?.App?.ImportPOC) {
          await window.go.main.App.ImportPOC(finalContent, category)
        }
      }
